...
새 프로젝트 트리 (Stage 1의 4 시스템 prefix와 합쳐서):
...
- [x] [src/lib/classify-question.ts](src/lib/classify-question.ts) **신규**:
  ...
  - 호출은 [llm-providers.ts](src/lib/llm-providers.ts) `streamChat` 또는 단발
    `runChat` 헬퍼 — Stage 4에서 자리 잡은 호출 패턴 재활용. (구현은 streamChat
    사용 + sweep-reviews.ts의 `extractJsonObject` 재활용으로 ```json 펜스도 허용.)
...
4. 줄어든 후보 위에서 [search.ts](src/lib/search.ts) `searchWiki`(또는 분기된
   `searchPaths`) 실행
5. 결과 + 트레이싱 블록 (judged type / 적용된 배제 / residue 수 / hits / 결과별
   sources / zeroResidueMeaning) 렌더
...
- [x] [src/components/search/search-view.tsx](src/components/search/search-view.tsx)
  갱신 — 기존 vector 검색 UI는 보존하되 결과 위에 **트레이싱 블록**을 collapsible로
  추가. 패턴은 [chat-message.tsx](src/components/chat/chat-message.tsx)의 think/
  citation collapsible 재활용 (PLAN.md §10 권장).
- [x] [src/components/search/exclusion-trace.tsx](src/components/search/exclusion-trace.tsx)
...
3. 사이드바 검색 → "Fireball을 SafeZone에서 발사할 때 어떻게 되는가?" 입력.
4. **확인:** 결과 위에 트레이싱 블록 — "판정된 유형: condition_based_test",
...
**목표:** 검색 1회마다 `exclusions/instances/<YYYY-MM>/<query-id>.md`에 trace를
markdown으로 영구 기록한다. git commit 1건 발생 → 사람이 git tooling으로 이력
조회 가능.

### 작업 항목

- [ ] [src/lib/instance-log.ts](src/lib/instance-log.ts) **신규**:
  - `recordSearchInstance(projectPath, question, result): Promise<{ path, commitHash }>`.
  - 경로 결정: `exclusions/instances/<YYYY-MM>/q-<ISO-timestamp>-<slug>.md`.
    `<slug>`는 질문에서 추출 (Stage 1에서 만든 slug 헬퍼 재활용).
  - markdown 본문: 질문 / 판정된 유형 / 적용된 entry별 매칭 path / residue 수 /
    hits 상위 N개 / 타임스탬프.
- [ ] [src/lib/auto-commit.ts](src/lib/auto-commit.ts) — `commitSearchInstance`
  추가. 메시지: `search: <type-id> → N hits (M excluded)`. 빈 hits도 기록(잔존 0
  자체가 의미 — §2.10).
- [ ] [src/lib/exclude-search.ts](src/lib/exclude-search.ts) — `runExcludeSearch`
  반환 직전에 `recordSearchInstance` 호출. 사용자가 검색 박스를 비워두는 짧은
  noise 입력은 record 생략 (질문 길이 임계값).
- [ ] [src/lib/__tests__/instance-log.test.ts](src/lib/__tests__/instance-log.test.ts)
  **신규** — 5 케이스: 파일 생성 + frontmatter / `<YYYY-MM>` 디렉토리 자동 생성 /
  잔존 0 케이스도 기록 / 동일 ms 타임스탬프 충돌 회피 / commit 호출 정합성.
...
2. 사이드바 검색에서 서로 다른 질문 3건을 순서대로 실행.
3. ```bash
...
- [ ] [src/components/exclusions/promotion-panel.tsx](src/components/exclusions/promotion-panel.tsx)
  **신규** — 사이드바에 "수정 요청" 다음 자리에 작은 진입점 또는 별도 view.
  카드: typeId + path + 빈도 + 인용된 instance N개. 액션 [Promote to Pattern] /
  [Promote to Axiom] / [Dismiss].
- [ ] [src/components/layout/icon-sidebar.tsx](src/components/layout/icon-sidebar.tsx)
  — promotion view 아이콘 추가 (`TrendingUp` 등). `nav.promotion` i18n 키 ko/en/zh.
...
1. Stage 12 검증을 통과한 프로젝트에서, 동일 (typeId, path) 쌍이 일치하는 검색을
   임계값 + 1 회 반복 실행.
...
6. 다음 검색 1회 실행 → Stage 11 트레이싱이 새 axiom/pattern을 적용하는지 확인.
...
3. **반례:** 검색 결과에서 이전에 배제됐던 path를 [정답으로 표시] →
   해당 배제 entry가 needs_review로 전환.
4. **Archive:** 한 entry [Archive] 클릭 → 다음 검색에서 적용 안 됨, 파일에는 보존.
   [Restore] → 다시 적용됨.
...
- [ ] [src/test-helpers/scenarios/search-scenarios.ts](src/test-helpers/scenarios/search-scenarios.ts)
  **신규** — game-dev 도메인 시나리오:
  - 부트스트랩 → 3개 question_type + 1개 axiom 작성 → 검색 5회 (동일 후보 4회 +
    다른 1회) → promotion 후보 1건 발견 → axiom 승격 → 다음 검색에서 axiom 적용
    트레이싱.
  - LLM 응답은 모킹.
...
2. `schema/question_types/`에서 `condition_based_test.md` 복사 → 프로젝트
   `question_types/`로.
3. "Fireball을 SafeZone에서 발사할 때 어떻게 되는가?" 검색.
4. **확인:** 트레이싱 블록이 IDEA §2.9의 예시 모양과 일치 — 판정 유형, 적용된
   배제, 탐색 시작 후보, 결과, 근거.

...
export function EntityView() {
  ...
  const [suggestions, setSuggestions] = useState<EntityMergeSuggestion[] | null>(null)
  ...
  const remove = async (entity:EntityDetail) => { const impact=await getKnowledgeDeleteImpact(projectPath,{entityId:entity.entityId});const ok=await confirm(`Delete “${entity.canonicalName}”?\n${impact.nodeIds.length} projections, ${impact.assertionIds.length} assertions and ${impact.pageIds.length} referenced pages are affected.`);if(ok){await deleteKnowledgeEntity(projectPath,entity.entityId,impact.revision);await reload()} }
  ...
  const loadSuggestions = async () => { setSuggestions(await suggestEntityMerges(projectPath)) }
  const mergeSuggestion = async (s:EntityMergeSuggestion) => { if(await confirm(`Merge “${s.a.canonicalName}” into “${s.b.canonicalName}”?`)){await mergeKnowledgeEntities(projectPath,s.a.entityId,s.b.entityId);setSuggestions((prev)=>prev?.filter((x)=>x.a.entityId!==s.a.entityId||x.b.entityId!==s.b.entityId)??null);await reload()} }

  return <div className="flex h-full flex-col overflow-hidden">
    <div className="flex gap-2 border-b p-2"><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search entities" className="flex-1 rounded border px-2 text-xs"/><input value={newName} onChange={(e)=>setNewName(e.target.value)} placeholder="Canonical name" className="rounded border px-2 text-xs"/><select value={newType} onChange={(e)=>setNewType(e.target.value as EntityType)} className="rounded border text-xs">{ENTITY_TYPES.map((t)=><option key={t}>{t}</option>)}</select><button onClick={()=>void add()} className="text-xs">+ Add</button><button onClick={()=>void loadSuggestions()} className="text-xs">Find duplicates</button></div>
    {suggestions!==null&&<div className="border-b bg-muted/30 p-2 text-xs"><div className="mb-1 flex items-center justify-between"><span className="font-medium">Merge suggestions ({suggestions.length})</span><button onClick={()=>setSuggestions(null)}>Hide</button></div>{suggestions.length===0?<div className="text-muted-foreground">No likely duplicates found.</div>:suggestions.map((s)=><div key={`${s.a.entityId}:${s.b.entityId}`} className="flex items-center gap-2 border-t py-1"><div className="flex-1"><span className="font-medium">{s.a.canonicalName}</span> ↔ <span className="font-medium">{s.b.canonicalName}</span> <span className="text-muted-foreground">({s.a.entityType} · {s.reason} · {(s.score*100).toFixed(0)}%)</span></div><button onClick={()=>void mergeSuggestion(s)}>Merge →</button></div>)}</div>}

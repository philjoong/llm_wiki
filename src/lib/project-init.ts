 ...
 * always exist by the time any ingest runs. ingest's sandbox check
...
/**
 * Bootstrap a freshly created project directory: ensure the system-prefix
 * directories exist with `.gitkeep` markers and seed the exclusion docs.
 *
 * Domain-specific subdirectories under `db/...` are not created here —
 * the ingest pipeline emits them on demand from source content.
 *

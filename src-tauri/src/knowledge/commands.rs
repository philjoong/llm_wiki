...
#[tauri::command]
pub fn discard_section_assertions(project_path:String,section_id:String)->Result<(),KnowledgeError>{
    let mut c=db::open_project(&project_path,true)?;
    let tx=c.transaction().map_err(sql)?;
    let assertion_ids:Vec<String>={let mut q=tx.prepare("SELECT DISTINCT assertion_id FROM assertion_evidence WHERE section_id=?1").map_err(sql)?;let rows=q.query_map([&section_id],|r|r.get(0)).map_err(sql)?.collect::<Result<Vec<_>,_>>().map_err(sql)?;rows};
    tx.execute("DELETE FROM assertion_evidence WHERE section_id=?1",[&section_id]).map_err(sql)?;
    for assertion_id in assertion_ids {
        let remaining:i64=tx.query_row("SELECT COUNT(*) FROM assertion_evidence WHERE assertion_id=?1",[&assertion_id],|r|r.get(0)).map_err(sql)?;
        if remaining==0 { tx.execute("DELETE FROM assertions WHERE assertion_id=?1",[&assertion_id]).map_err(sql)?; }
    }
    tx.commit().map_err(sql)?;
    Ok(())
}

...
[5] 확정된 db/ 트리 + 출처 추적 + git commit 이력
       ↓ 
[6] question_types/ + exclusions/ 작성
       ↓
...
`raw/sources/`와 `raw/originals/`는 Raw import 시점에 사용된다. 앱은 원본에서
markdown을 추출해 `raw/sources/<name>.md`에 저장하고, 사람이 원본을 다시 확인할
수 있도록 원본 파일은 gitignore 대상인 `raw/originals/`에 보관한다.
...
raw 파일
   ↓
[Step 0] raw/sources/<basename>.md   — 1차 가공 markdown
   ↓
...
```bash
find <projectPath>/raw/sources -name "*.md"
# → 업로드한 파일이 markdown으로 정규화되어 들어 있음
...
채팅에서 검색이 필요한 질문을 보내면 다음 흐름으로 동작한다.
...
`schema/question_types/`에 game-dev 도메인의 12개 카탈로그 예시(condition_based_test,
combination_explosion_test, policy_violation, regression_test 등)가 들어 있다. 새 프로젝트의
`question_types/`로 그대로 복사한 뒤 본문만 자기 도메인에 맞게 고쳐 쓰는 편이 빠르다.
...
(`promotion_rules.md`의 `pattern_min_count`, 기본 5)을 넘으면 사이드바
...
채팅 입력창에 질문을 보내면 assistant 메시지에 **트레이싱 블록**이 collapsible로
붙는다.
...
pattern_min_count: 5          # 같은 (type, path) 쌍이 N회 이상 instance에 등장하면 후보
axiom_min_patterns: 3         # axiom 후보 관련 임계값 (향후 확장용)
...
| Promotion 후보가 안 뜸 | 임계값 미달 또는 같은 후보가 dismiss됨 | `pattern_min_count` 낮추기 / `.llm-wiki/promotion-dismissals.jsonl` 검토 |
...
- [development-plan.md](development-plan.md) — 구현 이력과 현재 완료 상태 요약
- [schema/game-dev-example.md](schema/game-dev-example.md) — game-dev schema의
  사람이 읽는 reference
- `schema/question_types/` (저장소 내) — game-dev 기준 12개 question_type 카탈로그

...
export const searchScenarios: SearchScenario[] = [
  ...
  //    should rank first and have titleMatch=true.
  {
    ...
    initialWiki: {
      "db/attention.md": page("Attention", "The attention mechanism weights sequence tokens."),
      "db/other.md": page("Other Page", "Discusses something unrelated entirely."),
      "db/transformer.md": page("Transformer", "Uses attention across many heads."),
    },
    query: "attention",
    expected: {
      topResultPaths: ["db/attention.md"],
      titleMatchPaths: ["db/attention.md"],
      excludedPaths: ["db/other.md"],
    },
  },
  ...
  // 2. content-match — query not in title, still found via body text
  {
    ...
    initialWiki: {
      "db/embeddings.md": page(
        "Embeddings",
        "Rotary position embeddings inject positional information via rotation.",
      ),
      "db/other.md": page("Other", "Unrelated content here."),
    },
    query: "rotary",
    expected: {
      topResultPaths: ["db/embeddings.md"],
      excludedPaths: ["db/other.md"],
    },
  },
  ...
  // 3. cjk-bigram — Chinese query via bigram tokenization
  {
    ...
    initialWiki: {
      "db/attention-zh.md": page(
        "注意力机制",
        "注意力机制是 Transformer 架构的核心组件之一。",
      ),
      "db/unrelated-zh.md": page(
        "无关页面",
        "这个页面讨论别的话题，比如天气和足球。",
      ),
    },
    query: "注意力机制",
    expected: {
      topResultPaths: ["db/attention-zh.md"],
      excludedPaths: ["db/unrelated-zh.md"],
    },
  },
  ...
  // 4. multi-token-ranking — more matches ranks higher
  {
    ...
    initialWiki: {
      ...
      "db/only-attn.md": page("Attention", "Attention is a weighting mechanism."),
      "db/only-trans.md": page("Transformer", "Stacks of layers form the transformer."),
    },
    query: "attention transformer",
    expected: {
      topResultPaths: ["db/combined.md"],
    },
  },
  ...
  // 5. stop-word-filtered — 'the' filtered, only meaningful tokens used
  {
    ...
    initialWiki: {
      "db/attention.md": page("Attention", "Attention over keys and values."),
      "db/other.md": page("Other", "This page uses the word 'the' often but " +
  ...
  //    word many times. Regression guard for the "精准匹配排到最后" bug.
  {
    ...
    initialWiki: {
      "db/attention.md": page("Attention", "Attention is a weighting mechanism."),
      "db/multi-head-attention.md": page(
      ...
      "db/self-attention-history.md": page(
      ...
      "db/transformer-deep-dive.md": page(
    ...
    expected: {
      ...
      titleMatchPaths: [
        "db/attention.md",
        "db/multi-head-attention.md",
        "db/self-attention-history.md",
      ],
    },
  },
  ...
  //    raw query phrase should outrank a page with the same tokens scattered.
  {
    ...
    expected: {
      topResultPaths: ["db/phrase-page.md"],
    },
  },
]
